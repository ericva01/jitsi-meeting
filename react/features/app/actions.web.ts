...
export function maybeRedirectToWelcomePage(options: { feedbackSubmitted?: boolean; showThankYou?: boolean; } = {}) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        if (enableClosePage) {
            ...
            const displayName = state['features/base/settings'].displayName || 'You';
            ...
            window.sessionStorage.setItem('leavingDisplayName', displayName);
        ...
        if (options.showThankYou) {
            ...
            dispatch(showNotification({
                titleArguments: { name: displayName },
